...
def treinar_epocas_em_pastas(modelo, pasta_dados, epocas_por_pasta=10):
    for operacao in ['subtracao', 'adicao', 'multiplicacao', 'divisao']:
        ...
        X, y = prepare_data(dados)
        X = X.reshape((X.shape[0], 1, X.shape[1]))

        modelo.fit(X, y, epochs=epocas_por_pasta, batch_size=batch_size, validation_data=(X_validacao, y_validacao), callbacks=[model_checkpoint])
...
if __name__ == "__main__":
    ...
    # Treinamento por pastas
    treinar_epocas_em_pastas(modelo, 'dataset/treinamento', epocas_por_pasta=10)
